...
void initializeDefaultScene()
{
    ...
    spheres[1].Center = vec3(3.0, -1.0, -1.0);
    spheres[1].Radius = 1.2;
    spheres[1].MaterialIdx = 4;

    cubes[0].Center = vec3(-1.0, 1.0, 0.5);
    cubes[0].Size = 1.5;
...
{
    uLight.Position = vec3(0.0, 4.0, -4.0);
...
{
    uCamera.Position = vec3(0.0, 0.0, -5.0);
...
void main(void)
{
    ...
    pushRay(trRay);
    while(!isEmpty() && stack.container[stack.top].depth < DEPTH)
    {
